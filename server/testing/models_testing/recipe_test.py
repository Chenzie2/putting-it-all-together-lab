...
class TestRecipe:
    ...
    def create_user(self):
        '''Helper to create and return a user.'''
        user = User(
            username="testuser",
    ...
    def test_requires_title(self):
        ...
        with app.app_context():
            ...
            recipe = Recipe(
                instructions="Long enough instruction text to pass validation.",
                minutes_to_complete=30,
                user_id=user.id
            )

            recipe.title = None  # explicitly set to None
    ...
    def test_requires_50_plus_char_instructions(self):
        '''must raise error for instructions under 50 characters.'''
        with app.app_context():
            user = self.create_user()
